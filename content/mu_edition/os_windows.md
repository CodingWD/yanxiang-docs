...
In this section, we will use the Windows 11 recovery image file, which is the Windows 11 image pre-installed on the Innuo Mu from the factory. This image comes with all hardware drivers and is ready to use, providing you with a fast and ultimate experience.
...
??? tip "How to install the recovery OS image to the SSD?"
    - Insert your SSD into the Innuo Mu with the carrier board, then remove all other storage.
...
The Innuo Mu runs a full version of the Windows OS, which means that the installation process is the same as on a regular laptop or desktop computer. The general steps are as follows:
...
- Plug the USB drive into the Innuo, and continuously press **++del++** after turning it on to enter the BIOS settings. Configure the Innuo to boot from the USB drive.
