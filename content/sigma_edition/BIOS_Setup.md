...
* Press the **++del++** key repeatedly before the Innuo logo appears on the screen during the boot process.
...
All four USB Type-A ports on Innuo Sigma support power control. The Font USB 2.0 port, rear USB 3.2 gen2 port and the USB 2.0 pin header can be set separately.
...
!!! Note
    When the Innuo is disconnected from the power supply, all USB-A ports and the MCU power control are powered off by default. When the Innuo is reconnected to the power supply and not turned on, the USB-A ports and the MCU power control remain off.


## Auto Power-on
To turn on Innuo without pressing the power button, please set Auto Power On in the BIOS.
