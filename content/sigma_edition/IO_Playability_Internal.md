...
There are four M.2 sockets on Innuo 3 Delta: **M key x2, E key x1, B key x1**.
...
=== "M.2 B Key"
    ...
    !!! note ""
        ...
        #### <span style="color: green">:material-check-circle:</span> **Compatible Device**
          ...
          Before using the 4G or 5G module, please insert a micro-SIM card into the SIM card slot, then assemble the antennas on the module. More information to get Cecullar network on Innuo, pls refer to [**Multiple Network**](Internet_Connection.md#cellular-4g5g).
...
One of the distinctive features of Innuo is its built-in co-processor, the Arduino Leonardo, which is a microcontroller based on ATmega32u4.
...
By default, the Arduino Leonardo pins on the Innuo Sigma are powered only when the device is either turned on or in sleep mode. However, **enabling the MCU Power Control** in the BIOS settings will keep them powered even when the device is off.
...
By default, the 5V Pin Headers on the Innuo Sigma are powered only when the device is either turned on or in sleep mode. However, enabling the **'Always On 5V Pin Header'** option in the BIOS settings will keep them powered even when the device is off.
...
By shorting these pins, you can turn the Innuo on or off. 

#### :material-restart: Reset Switch Pins
By shorting these pins, you can reset the Innuo.
...
The Innuo Sigma features shared pinout design for its RS232 and RS485 serial interfaces. These functions are controlled by **a DIP switch** located on the back side of Sigma board, near the RTC battery.
