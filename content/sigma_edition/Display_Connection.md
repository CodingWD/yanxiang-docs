...
This chapter will demonstrate multiple ways to connect display devices to your Innuo, and provide information on the necessary components and installation steps.

!!! Warning "Electrostatic Warning"

    Be sure to remove the static electricity from your body before touching the Innuo! Otherwise, it may cause potential static discharge and damage your Innuo! 
...
!!! Note "4K HDR"
    ...
    The Innuo Sigma supports HDMI 2.1, therefore, if you are using a display that supports 4K 60Hz HDR, it is recommended to use an HDMI 2.0b or 2.1 cable.
...
1. Connect one end of the HDMI cable to the HDMI port on the Innuo.
2. Connect the other end of the HDMI cable to the TV or monitor.

## USB Type C

The USB Type C ports on Innuo Sigma are both full function Thunderbolt 4. They both supports power, USB data, video and PCIe.
...
??? Info "Thunderbolt 4 Introcution"
    ...
    - Thunderbolt 4 also supports daisy-chaining multiple devices together. You can connect multiple displays to a single Thunderbolt 4 port, without needing to use additional video ports on Innuo. 
...
### Steps
   ...
   - If your display device does not have a USB Type C port, you will need to use a USB Type C Dock. Follow these steps to connect your Innuo to the display device:

    > a. Locate the USB Type C port on your Innuo(there are two USB Type C ports on Innuo Sigma, both of which can be used).
    
    > b. Connect the USB Type C HUB to the USB Type C port of your Innuo. 
   ...
   - If your display device has a USB Type C port, you can directly connect your Innuo using a USB Type C cable. Follow these steps:

    > a. Locate the USB Type C port on your Innuo.
    
    > b. Connect one end of the USB Type C cable to the USB Type C port of your Innuo. Make sure it is securely plugged in.
...
![type_C_display](../../assets/images/Innuo 3 Delta/type C display_3 delta.webp)

## eDP(Embedded DisplayPort)

The Innuo Sigma uses a <span style="color: rgba(225, 103, 13);">**0.5mm pitch 40-pin connector**</span> for its eDP interface, which has 4 lanes and supports resolutions up to 4K and refresh rates up to 120Hz. This design allows for a power-efficient and high-bandwidth display connection while also enabling thinner and lighter device designs. 

### Compatibility

In most cases, display devices equipped with a 40-pin eDP connector and powered by 3.3V are compatible with the Innuo Sigma and do not necessitate the installation of additional drivers. Notably, the eDP connector on the Innuo Sigma is designed with four lanes.

### Preparations

- [Touch Display (eDP) for Innuo Sigma](https://www.dfrobot.com/product-2763.html)
...
!!! Warning "Ensure Correct eDP Cable Installation"
    ...
    To avoid any potential damage to your Innuo or display device, it's important to ensure that the eDP cable is correctly installed and the display is connected before turning on your Innuo. Please double-check these connections carefully, as failure to do so may cause a short circuit.

- Before starting, ensure that your Innuo is powered off and disconnected from its power supply cable.

- Align the eDP cable with the eDP connector on Innuo. Make sure the gold finger of the eDP cable face up.
...
- Reference Video: :fontawesome-brands-youtube:{ style="color: #EE0F0F" }[How to Install the 7'' eDP Touch Display for Innuo Sigma](https://youtu.be/tbvC4nlbtog)
