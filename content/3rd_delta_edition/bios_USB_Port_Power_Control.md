...
Three USB Type-A ports on Innuo 3 Delta support power control. Every port can be configured independently.

**Path: BIOS Setup --> Advanced --> USB Configuration --> Always On USB Port**

  ![](../../assets/images/Innuo 3 Delta/USBcontrol_BIOS_3Delta.webp){width="600" }
...
|                    | <span style="color: green">:fontawesome-solid-check:</span> means **Enabled**, the USB port can output power;<br> :x: means **Disabled** the USB port can't output power. |
  ...
  ![](../../assets/images/Innuo 3 Delta/usb_number_3Delta.webp){width="600" }

!!! info "**Attention:** "

    - If Innuo is disconnected from the power supply, all USB A ports are powered off by default when the Innuo connects power supply again and is not turned on. The MCU power control is also the same.
