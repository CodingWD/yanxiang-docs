...
There are two M.2 sockets on Innuo 3 Delta: M key and B key.

![](../../assets/images/Innuo 3 Delta/M2sockets_3Delta.webp)
...
=== "M.2 B Key"
    ...
    !!! note ""
        ...
        #### <span style="color: green">:material-check-circle:</span> **Compatible Device**
          ...
          Before using the 4G or 5G module, please insert a micro-SIM card into the SIM card slot. More information to get Cecullar network on Innuo, pls refer to [**Multiple Network**](connectivity.md#cellular-4g5g).
...
![](../../assets/images/Innuo 3 Delta/StatePins_3Delta.webp){width="300"}
...
![](../../assets/images/Innuo 3 Delta/PowerControlPins_3Delta.webp){width="300"}
...
![](../../assets/images/Innuo 3 Delta/Auto5VPowerPin_3Delta.webp){width="400"}
...
Innuo 3 Delta has two-channel audio output pins with audio power amplifier and the output power of each channel can be <span style="color: rgba(225, 103, 13);">**up to 2Watt (rms)/4Ω**</span>. You can directly connect two speakers to play sound.

When the headphone is inserted, the audio output automatically switches to the headphone jack.

![](../../assets/images/Innuo 3 Delta/audioput_pin_3Delta.webp){width="400" }

### :material-battery-clock: External RTC Power Pin

The Innuo 3 Delta comes equipped with an onboard CR927 RTC battery, with a voltage of 3.0V, capable of maintaining power for **around 9~12 months**.
...
![](../../assets/images/Innuo 3 Delta/ExteranlRTCPowerPin_3Delta.webp){width="300"}

!!! note ""
    #### Note for Old Version Users
    If you are using the old version of the Innuo 3 Delta, be aware that the status of the RTC battery will affect the Auto Power-On function. For more details, refer to the section on the [**Auto Power-On Function**](../3rd_delta_edition/bios_Auto_power_on.md) in the BIOS Setup.


### GPIO Pin
#### :simple-arduino: Arduino Leonardo Pins
Innuo 3 Delta has an ATmega32u4 chip on board, and the built-in firmware is compatible with Innuo Leonardo. It is connected to the processor through the USB 2.0 lane, and is mapped to a COM port device in the operating system, named Innuo Leonardo. 

![](../../assets/images/Innuo 3 Delta/GPIO_3Delta.webp){width="300"}
...
By default, the Arduino Leonardo pins on the Innuo 3 Delta are powered only when the device is either turned on or in sleep mode. However, **enabling the MCU Power Control** in the BIOS settings will keep them powered even when the device is off.

**Path: BIOS menu --> Advanced --> Power Management --> MCU Power Control**

![BIOS setting MCU Power Control](../../assets/images/Innuo 3 Delta/3_delta_MCU_POWER.webp)
...
!!! note
    **This setup only functions with a constant power supply.** If the power is interrupted, the Arduino pins will not receive power until the device is rebooted. To ensure the pins remain powered whenever the device is connected to a power source, you can connect the 5V pins as shown below, allowing the DC 5V to continuously power the Arduino.
    ![](../../assets/images/Innuo 3 Delta/5V_pin.webp)
...
- Recommended Cable: [Innuo DC 5.5x2.1mm Power Cable](https://www.dfrobot.com/product-2624.html)
...
Innuo 3 Delta is equipped with a 30-pin eDP connector and I2C touch connector. Please refer to [**eDP(Embedded DisplayPort**)](touch_and_display.md#edpembedded-displayport) chapter to learn how to use the eDP touch display.
