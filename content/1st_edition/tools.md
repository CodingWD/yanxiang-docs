...
Using a VNC server will enable you to access the GUI of Windows running on your Innuo from a different PC on your local network. TightVNC is a free and easy way to set up this service.
...
1.Download and install TightVNC for Windows on your Innuo. Choose 32-bit or 64-bit depending on your system architecture. (Innuo Standard is 32-bit, Innuo Enhanced is 64-bit)
...
3.Choose Setup Type
   ...
   Typical installation will install both TightVNC server and TightVNC viewer on your Innuo

   Custom installation allows you to select which elements to install. In this situation, all we need is the server itself (unless you would like to be able to view other PCs on your network through the Innuo, in which case you will need to install the viewer, as well).
...
Next, you will need to go on to the computer that you would like to use to control the Innuo with, and using the same installation package install TightVNC viewer. When you have successfully installed TightVNC viewer, check that you are on the same network as your Innuo, otherwise it will not work!

When you are certain that your computer and the Innuo are on the same local network, proceed to step 3

### Step 3 - Testing

   ![vnc_image009](../../assets/images/LP V1/V1_Tools/vnc_image009.webp)

Open TightVNC Viewer. A window will appear for a new TightVNC Connection. At this point, you need to input the IP address of your Innuo.

   ![vnc_image010](../../assets/images/LP V1/V1_Tools/vnc_image010.webp)

Tip: A quick way to find this IP address is to hover over the system tray TightVNC icon on your Innuo. A hint will pop up with “TightVNC Service - 
...
” You can also go into your router control interface and look for attached devices. The next step is to input this IP address into the New TightVNC Connection Window, followed by the port number you set in the service settings. The default port is 5900.
 ...
 `<ip address of Innuo>:<port number>`
...
You will now see a window containing your Innuo’s GUI! You can now control it remotely!

![vnc_image012](../../assets/images/LP V1/V1_Tools/vnc_image012.webp)

This concludes the Innuo VNC tutorial. If you have any questions or comments, please let us know in the forums. We hope this has has helped.
